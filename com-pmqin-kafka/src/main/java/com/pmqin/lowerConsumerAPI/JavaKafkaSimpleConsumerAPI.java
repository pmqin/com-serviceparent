package com.pmqin.lowerConsumerAPI;
...
/**
 * TODO: 添加必要的日志打印信息 http://www.cnblogs.com/liuming1992/p/6432506.html
 * http://blog.csdn.net/qq_35799003/article/details/52230193 Kafka Lower
 * consumer api ==> Kafka Simple Consumer API
 * http://blog.csdn.net/suifeng3051/article/details/38657465
 * http://blog.csdn.net/qq_35799003/article/details/52230193
 */
public class JavaKafkaSimpleConsumerAPI {
	private static final Logger logger = LogManager.getLogger(JavaKafkaSimpleConsumerAPI.class);

	public static void main(String[] args) {
		JavaKafkaSimpleConsumerAPI example = new JavaKafkaSimpleConsumerAPI();
		long maxReads = 300;
		String topic = "test-topic";// "pmqin";
		int partitionID = 0;
	 ...
	 * @throws Exception
	 */
	public void run(long maxReads, KafkaTopicPartitionInfo topicPartitionInfo, List<KafkaBrokerInfo> seedBrokers)
			throws Exception {
		...
		SimpleConsumer consumer = this.createSimpleConsumer(metadata.leader().host(), metadata.leader().port(),
				clientName);
		...
		try {
			...
			while (ever || maxReads > 0) {
				...
				// 重置失败次数
				numErrors = 0;
				//response.messageSet(topic, partitionID).iterator();
				...
				for (MessageAndOffset messageAndOffset : response.messageSet(topic, partitionID)) {
					...
					System.out.println("messageAndOffset.nextOffset():" + readOffSet);
					// 读取数据的value
				...
				// 更新偏移量
				consumer = this.updateOffset(consumer, topic, partitionID, readOffSet, groupId, clientName, 0);
	 ...
	 *
	 * @param brokers
	 *            Kafka集群连接参数，eg: {"hadoop-senior01" -> 9092, "hadoop-senior02"
	 *            -> 9092}
	 * @param topic
	 *            topic名称
	 * @param partitionID
	...
	public PartitionMetadata findLeader(List<KafkaBrokerInfo> brokers, String topic, int partitionID) {
		...
		for (KafkaBrokerInfo broker : brokers) {
			...
			try {
				...
				System.out.println("Error communicating with Broker [" + broker.brokerHost + "] to find Leader for ["
	 ...
	 *            所属的分区ID
	 * @param whichTime
	 *            用于判断，当consumer从没有消费数据的时候，从当前topic的Partition的那个offset开始读取数据
	 * @param clientName
	 *            client名称
	...
	public long getLastOffSet(SimpleConsumer consumer, String groupId, String topic, int partitionID, long whichTime,
		...
		if (response.hasError()) {
			System.out.println(
					"Error fetching data Offset Data the Broker. Reason: " + response.errorCode(topic, partitionID));
			return -1;
		}
	 ...
	 * @return
	 */
	public long getOffsetOfTopicAndPartition(SimpleConsumer consumer, String groupId, String clientName, String topic,
			int partitionID) {
		TopicAndPartition topicAndPartition = new TopicAndPartition(topic, partitionID);
		...
		if (returnOffsetMetadata != null && !returnOffsetMetadata.isEmpty()) {
			...
			if (offset.error() == ErrorMapping.NoError()) {
				...
				System.out
						.println("Error fetching data Offset Data the Topic and Partition. Reason: " + offset.error());
			}
		}
	 ...
	 *             当更新失败的情况下
	 */
	private SimpleConsumer updateOffset(SimpleConsumer consumer, String topic, int partitionID, long readOffSet,
			String groupId, String clientName, int times) {
		// 构建请求对象
		...
		if (response.hasError()) {
			...
			if (times > this.maxRetryTimes) {
				throw new RuntimeException(
						"Update the Offset occur exception," + " the current response code is:" + code);
			}
			...
			// 其他code直接抛出异常
			throw new RuntimeException("Update the Offset occur exception," + " the current response code is:" + code);
		}
	...
	private SimpleConsumer createNewSimpleConsumer(SimpleConsumer consumer, String topic, int partitionID) {
		...
		this.validatePartitionMetadata(metadata);
		// 重新创建consumer的连接
		return this.createSimpleConsumer(metadata.leader().host(), metadata.leader().port(), consumer.clientId());
	}
	 ...
	 * @return
	 */
	public static List<Integer> fetchTopicPartitionIDs(List<KafkaBrokerInfo> brokers, String topic, int soTimeout,
